...
class Company(models.Model):
    ...
    price_low_since = models.DateTimeField(null=True, blank=True)

    def __str__(self):
        return self.name

    def save(self, *args, **kwargs):
        if self.current_stock_price < Decimal('1.50'):
            if not self.price_low_since:
                self.price_low_since = timezone.now()
        else:
            # Reset this field if the price goes above $1
            self.price_low_since = None
